...
\subsection{Prefernce Distribution and Noise}

We create 100 voters with different preferences over the 10 available candidates. We have 2 different distributions from which the preferences are created. First there is a uniform (0,1) distribution over all of the candidates. Second we simulated a having preferences selected over a normal prior with mean as the candidate number divided by the total number of candidates. Thus certain candidates are more likely to be preferred over others. For the candidates that are more preferred they also have a higher standard deviation of the candidate number divided by half the total number of candidates plus 1. We believe that candidates that are more preferred are more likely to have disagreement over their value.

In a practical sense, ten candidates appears perfectly rational to have as the total number of candidates. However in most cases the race will simply be between the two leading parties in a "tight" election. To simulate this tightness we continue to create preferences as above; next we replace the preferences for candidates 1 and 2 with an exponential prior with $\lambda = \frac{1}{2}$. Over 100 voters, one of candidate 1 or 2 now is most likely to be the socially optimal candidate.
...
Candidate preferences were sampled from a standard uniform distribution and a shifting Normal distribution. For candidate $i$, the preference was sampled from a $N(\sqrt(i), (i + 1)^2)$ distribution -- thus, candidates with higher ID numbers were slightly more preferred by the general population. In noisy election, each preference had standard Normal noise added to it. Finally, for ``tight'' elections, candidates 1 and 2 were selected to be favored and preferences for both candidates sampled from a $\Expo(2)$. 
\subsection{Results}
...
From the visualization above, we observe that in a truthful election, Borda Count does noticeably better than the other mechanisms. However, when noise is added to each voter's preference, we see that the utility loss of each election increases and also ...
